...
private void preReqs() {
    ...
    if (isSdPresent) { 		// Make sure FRC directory is there
        ...
        File directFRC = new File(Environment.getExternalStorageDirectory() + "/download/FRC514");
        if(!directFRC.exists())  {
            if(directFRC.mkdir())
            { }        //directory is created;
        }
        File directImg = new File(Environment.getExternalStorageDirectory() + "/download/FRC514/images");
        if(!directImg.exists())  {
            if(directImg.mkdir())
            { }        //directory is created;
        }
        File directMatch = new File(Environment.getExternalStorageDirectory() + "/download/FRC514/match");
        if(!directMatch.exists())  {
            if(directMatch.mkdir())
            { }        //directory is created;
        }
        File directPit = new File(Environment.getExternalStorageDirectory() + "/download/FRC514/pit");
        if(!directPit.exists())  {
            if(directPit.mkdir())
            { }        //directory is created;
        }
